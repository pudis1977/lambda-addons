...
class link:
    def __init__(self):
        ...
        self.livetv_nhlteams = 'http://livetv.sx/en/tables/2'
        self.livetv_nhlmatch = '/en/videotourney/2/'
...
class pages:
    ...
    def games(self):
        self.list = self.livetv_list(link().livetv_nhl, addonGames)
        index().pageList(self.list)

    def highlights(self):
        self.list = self.livetv_list(link().livetv_nhl, addonHighlights)
        index().pageList(self.list)

    def teams(self):
        #self.list = self.livetv_list2(link().livetv_nhlteams, addonTeams)
        self.list = cache2(self.livetv_list2, link().livetv_nhlteams, addonTeams)
        index().pageList(self.list)
...
class videos:
        ...
        for video in videos:
            try:
                ...
                try:
                    ...
                    pass
                try:
                    u = re.compile('.*href="(.+?)">First Period<').findall(video)[0]
                    url.update({'firstperiod': link().livetv_base + u})
                except:
                    pass
                try:
                    u = re.compile('.*href="(.+?)">Second Period<').findall(video)[0]
                    url.update({'secondperiod': link().livetv_base + u})
                except:
                    pass
                try:
                    u = re.compile('.*href="(.+?)">Third Period<').findall(video)[0]
                    url.update({'thirdperiod': link().livetv_base + u})
                except:
                    pass
                try:
    ...
    def livetv_list2(self, url):
        try:
            ...
            result = [i for i in result if i.startswith(' href="%s"' % link().livetv_nhlmatch)][-1]
...
class videoparts:
    ...
    def livetv_list(self, name, url, image, date, genre, plot, title, show):
        ...
        try:
            ...
            pass
        try:
            self.list.append({'name': '%s (1)' % title, 'url': i['firstperiod'], 'image': image, 'date': date, 'genre': genre, 'plot': plot, 'title': title, 'show': show})
        except:
            pass
        try:
            self.list.append({'name': '%s (2)' % title, 'url': i['secondperiod'], 'image': image, 'date': date, 'genre': genre, 'plot': plot, 'title': title, 'show': show})
        except:
            pass
        try:
            self.list.append({'name': '%s (3)' % title, 'url': i['thirdperiod'], 'image': image, 'date': date, 'genre': genre, 'plot': plot, 'title': title, 'show': show})
        except:
            pass
        try:
...
class resolver:
    ...
    def livetv(self, url):
        ...
        try:
            nhl_com = re.compile('/video.nhl.com.+?playlist=(.+?)"').findall(r)[0]
            nhl_com = 'http://video.nhl.com/videocenter/servlets/playlist?ids=%s&format=json' % nhl_com
            result = getUrl(nhl_com).result
